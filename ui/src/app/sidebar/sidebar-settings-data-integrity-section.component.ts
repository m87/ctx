    ...
    <div class="space-y-5">
      ...
      @if (integrityReport(); as report) {
        @if (hasRepairableIssues(report)) {
          ...
          >
            {{ repairIntegrityMutation.isPending() ? 'Repairing...' : 'Repair automatically' }}
          </button>
        }
      }
              ...
              <div class="rounded-md border p-3 text-[13px]">
                ...
                @if (group.hiddenIssueCount > 0) {
                  ...
                  >
                    ...
                    this {{ issue.entityType }}. Resolve this first step, then run the check again.
                  </div>
                }
                ...
                @if (isContextAssignmentIssue(issue)) {
                  <div class="mt-3 flex flex-col sm:flex-row gap-2">
                    <select
                      ...
                      "
                      (change)="selectIntegrityContext(issue.entityId, getSelectValue($event))"
                    >
                      <option value="">
                        {{
                    ...
                    >
                      {{ moveIntegrityIntervalMutation.isPending() ? 'Assigning...' : 'Assign' }}
                    </button>
                  </div>
                }
                ...
                @if (isIntervalTimeEditIssue(issue)) {
                  <div class="mt-3 rounded-md border bg-muted/20 p-3">
                    ...
                    <div class="flex flex-col sm:flex-row gap-2 sm:items-end">
                      ...
                      >
                        {{
                          updateIntegrityIntervalMutation.isPending() ? 'Saving...' : 'Save time'
                        }}
                      </button>
                    </div>
