...
func (m *SettingsManager) Save(settings map[string]string) error {
	s := &Settings{
		...
		general: GeneralSettings{
			...
			firstDay: settings["general.firstDay"],
		},
	}
...
func (m *SettingsMapper) ToNode(settings *Settings) (*nod.Node, error) {
	node := &nod.Node{
		...
		KV: map[string]*nod.KV{
			"general.theme":    &nod.KV{Key: "general.theme", ValueText: &settings.general.theme},
			"general.firstDay": &nod.KV{Key: "general.firstDay", ValueText: &settings.general.firstDay},
		},
	}
	return node, nil
}
...
func (m *SettingsMapper) FromNode(node *nod.Node) (*Settings, error) {
	return &Settings{
		general: GeneralSettings{
			theme:    *node.KV["general.theme"].ValueText,
			firstDay: *node.KV["general.firstDay"].ValueText,
		},
	}, nil
}
